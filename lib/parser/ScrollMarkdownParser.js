...
const ScrollMarkdownParser = function (tagloader, opts) {
...
ScrollMarkdownParser.prototype.compile = function (callback) {
    ...
    for (let i in this.tags) {
        ...
        let tag = this.tags[i];
        let markdown = tag.info.markdown;
        let aliases = [];
        let parents = [];
        ...
        if (markdown) {
            if (markdown.type === "block") {
                ...
                if (markdown.block_prefix) {
                    let open = markdown.block_prefix || '';
                    let close = PARA_CLOSE;
                ...
                let open_close = markdown.markdown.split("$");
