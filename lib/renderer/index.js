...
lodash.str = {clean: str => lodash.trim(str).replace(/\s\s+/g, ' ')};
...
Renderer.prototype.render_to_string = function (text, parser, callback) {
    ...
    let tag_class_tack = [];
    let _this = this;
    let emit = lodash.bind(result.push, result);
...
let EditorRenderer = function (tagloader, opts) {
    let defs = {
        //attach_source: true,
        tagloader: tagloader,
...
EditorRenderer.prototype.compile = function (callback) {
    ...
    this.html_tag_open = {};
    this.html_tag_close = {};
    let tags = this.tagloader.all_tags;
    for (let i in tags) {
        let tag = tags[i];
...
EditorRenderer.prototype._emit_open = function (emit, tag_class, text_content) {
    ...
    if (text_content) {
        ...
        if (this.normalize_source) {
            emit(lodash.str.clean(escape_quotes(text_content)));
        } else {
 ...
 */
let StyleRenderer = function (tagloader, style, opts) {
    let defs = {
        tagloader: tagloader,
        style: style,
